...
extern "C" void disassemble(char *str, int size, uint64_t pc, uint8_t *code, int nbyte) {
  ...
  llvm::MCDisassembler::DecodeStatus S = gDisassembler->getInstruction(inst, dummy_size, arr, pc, Annotations);

  if (S == llvm::MCDisassembler::Success) {
    // std::cout << "Successfully disassembled " << dummy_size << " bytes." << std::endl;
  } else {
  ...
  gIP->printInst(&inst, pc, Annotations.str(), *gSTI, os);
